<script setup>
import { STORE_VIDEO as STORE_VIDEO_PARENT } from "@/services/stores.js";
import { computed, ref } from "vue";

const accountInformation = JSON.parse(localStorage.getItem("token"));

const STORE_VIDEO = STORE_VIDEO_PARENT.StoreVideo();
...
const onClickIconLike = (id, listEmail) => {
  ...
  STORE_VIDEO.onActionLike(id, likedEmail);
};

// --------------------------------------------------------

// CALL API
const getListPostsVideo = () => {
  STORE_VIDEO.onActionGetListPostsVideo();
};
getListPostsVideo();

// GET DATA
const dataListPostsPHOTOComputed = computed(
  () => STORE_VIDEO.onGetterListsPostsVideo.value
);
</script>
