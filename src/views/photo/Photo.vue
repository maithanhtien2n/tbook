<script setup>
import { STORE_PHOTO as STORE_PHOTO_PARENT } from "@/services/stores.js";
import { computed } from "vue";

const accountInformation = JSON.parse(localStorage.getItem("token"));

const STORE_PHOTO = STORE_PHOTO_PARENT.StorePhoto();
...
const onClickIconLike = (id, listEmail) => {
  ...
  STORE_PHOTO.onActionLike(id, likedEmail);
};

// --------------------------------------------------------

// CALL API
const getListPostsPHOTO = () => {
  STORE_PHOTO.onActionGetListPostsPhoto();
};
getListPostsPHOTO();

// GET DATA
const dataListPostsPhotoComputed = computed(
  () => STORE_PHOTO.onGetterListsPostsPhoto.value
);
</script>
...
<template>
  <div class="col-5 p-0 flex flex-column gap-5">
    ...
    <div class="flex flex-column gap-5">
      <div
        v-for="(posts, index) in dataListPostsPhotoComputed"
        :key="index"
